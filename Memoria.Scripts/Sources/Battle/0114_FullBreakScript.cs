    ...
    public sealed class FullBreakScript : IBattleScript, IEstimateBattleScript
    {
        ...
        public void Perform()
        {
            ...
            if (!_v.Target.TryKillFrozen())
            {
                ...
                _v.BonusElement();
                if (TranceSeekCustomAPI.TryMagicHitWithoutBattleCalcFlag(_v))
                {
                    if (TranceSeekCustomAPI.SPSSpecialStatus[_v.Target.Data][0] == -1)
                    {
                        TranceSeekCustomAPI.AddSpecialSPS(_v.Target.Data, 0, -1, 1.0f);
                    }
                    if (TranceSeekCustomAPI.SPSSpecialStatus[_v.Target.Data][1] == -1)
                    {
                        TranceSeekCustomAPI.AddSpecialSPS(_v.Target.Data, 1, -1, 1.0f);
                    }
                    if (TranceSeekCustomAPI.SPSSpecialStatus[_v.Target.Data][2] == -1)
                    {
                        TranceSeekCustomAPI.AddSpecialSPS(_v.Target.Data, 2, -1, 1.0f);
                    }
                    if (TranceSeekCustomAPI.SPSSpecialStatus[_v.Target.Data][3] == -1)
                    {
