...
namespace Memoria.Scripts.Battle
{
    ...
    public sealed class DetectScript : IBattleScript
    {
        ...
        public void Perform()
        {
            if (_v.Command.AbilityId == BattleAbilityId.Detect)
            {
                Boolean VanishRemove = false;
                if (_v.Target.IsUnderAnyStatus(BattleStatusId.Vanish))
                {
                    _v.Target.RemoveStatus(BattleStatusId.Vanish);
                    VanishRemove = true;
                }
                ...
                _v.TargetState().Zidane.EyeOfTheThief = true;
            }
            else
            {
