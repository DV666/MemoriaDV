            ...
            if (_v.Caster.PlayerIndex == CharacterId.Amarant)
            {
                if (_v.Command.AbilityId == BattleAbilityId.NoMercy1 || _v.Command.AbilityId == BattleAbilityId.NoMercy2) // Fragilité
                {
                    ...
                    for (Int32 index = 0; index < 32; ++index)
                    {
                        ...
                        if ((_v.Target.Data.stat.cur & status) != 0)
                        {
                            if ((status & (BattleStatus.GradualPetrify | BattleStatus.Doom)) != 0)
                            {
                                _v.Target.Data.stat.cnt.conti[index] /= 2;
                            }
                            else
                            {
                                _v.Target.Data.stat.cnt.conti[index] *= 2;
                    ...
                    if (_v.Target.IsUnderAnyStatus(BattleStatus.Virus) && TranceSeekCustomAPI.SPSSpecialStatus[_v.Target.Data][32] == -1)
                        TranceSeekCustomAPI.AddSpecialSPS(_v.Target.Data, 32, -1, 1.0f);                   
                }
